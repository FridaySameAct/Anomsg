...
รัน 90 เคส ไม่ต้องใช้ token จริงหรือต่อเน็ต แบ่งเป็น 3 ชุด:
...
- `test/interactions.test.mjs` — เรียก handler ตรงๆ (48 เคส) ครอบคลุม PING/PONG,
  ลายเซ็นผิดต้องได้ 401, `/send` ทั้งแบบข้อความและแนบรูป, ไฟล์ที่ไม่ใช่ภาพ,
  ไฟล์เกิน 8MB, Discord ตอบ 401/403/404, `/ping`, คำสั่งไม่รู้จัก, body พัง
  และ env var หาย
