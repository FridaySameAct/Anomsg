...
// กัน sibling-subdomain (เช่น *.vercel.app ตัวอื่น) ตั้ง cookie ชื่อเดียวกันมายัดใส่ victim ได้ —
// เดิม cookie นี้ไม่มี prefix ในขณะที่ state cookie มี ทำให้คนที่คุม sibling subdomain ยัด
// anomsg_session ปลอมทับของจริงแล้ว log out เหยื่อได้ตามใจชอบ (ปลอม HMAC ไม่ได้ แต่ deny service ได้)
export const SESSION_COOKIE = '__Host-anomsg_session';

// __Host- เหมือนกับ SESSION_COOKIE ด้านบน ป้องกัน sibling-subdomain ยัด state ปลอมมาทำ
// session fixation ตอน login — ย้ายมารวมไว้จุดเดียวกับ session cookie กัน login.js/callback.js/logout.js
// ก็อปปี้ชื่อแยกกัน 3 ที่แล้ววันหนึ่งมีคนแก้ไม่ครบ
export const STATE_COOKIE = '__Host-anomsg_oauth';
...
export function verifySession(value, secret) {
  ...
  try {
    ...
    // บวก 60 วินาที เพื่อให้ offline clock skew ไม่ทำให้ session ใหม่ถูกปฏิเสธทันที
    if (!Number.isFinite(data.exp) || data.exp < now || data.exp > now + MAX_AGE_SECONDS + 60) return null;
...
// ต้องเคลียร์ cookie นี้ทุก terminal path ของ oauth flow ไม่ใช่แค่ทาง success ไม่งั้นมัน replay ซ้ำได้
// ตลอดอายุ 600 วิที่เหลือ — เดิมฟังก์ชันนี้ก็อปปี้แยกไว้ 3 ไฟล์ (login/callback/logout) ย้ายมารวมที่เดียว
