...
export function errorResponse(err) {
  ...
  // MONGODB_URI ตั้งไว้ แต่ Atlas ต่อไม่ติด (ล่ม/เน็ตขาด) ต่างจากกรณี isTasksEnabled() ที่ตรวจแค่ว่า
  // ตั้งค่าไว้หรือยัง — ต้องแยกออกมาตอบ 503 เหมือนกันเพราะเป็นปัญหาชั่วคราวที่ retry แล้วอาจหายเอง
  // ไม่ใช่ bug ของเรา ไม่ควรตอบ 500 ปนกับ error ที่ไม่คาดคิดจริงๆ — MongooseServerSelectionError คือ
  // error class เดียวที่ mongoose โยนตอน connect()/query หา server ไม่เจอเลยภายใน serverSelectionTimeout
  if (err instanceof mongoose.Error.MongooseServerSelectionError) {
    // ยัง log ไว้ฝั่งเซิร์ฟเวอร์เพื่อ debug แต่ห้ามส่ง err.message ออกไปใน body เพราะมันมักมี host/connection
    // string ปนอยู่ (ดู isAtlasWhitelistError ใน mongoose) — ใช้ข้อความคงที่แบบเดียวกับที่ /task และ
    // /mytask ใช้ตอบ ephemeral เมื่อ DB ล่ม (lib/task.js, lib/mytask.js) ให้ผู้ใช้เห็นข้อความสม่ำเสมอ
    console.error('db connection failed:', err);
    return jsonNoStore({ error: 'ระบบ task ใช้ไม่ได้ชั่วคราว ลองใหม่อีกครั้ง' }, { status: 503 });
  }
